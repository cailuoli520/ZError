...
export interface AppSettings {
  ...
  adminToken: string
  /** 公网 /query 是否必须携带令牌 */
  publicQueryRequireToken: boolean
  multiUser: {
    enabled: boolean
    users: UserConfig[]
  }
...
class SettingsManager {
  ...
  /**
   * 登录后从服务端加载设置（服务端为唯一权威存储）
   */
  async init(): Promise<void> {
    if (!getAdminToken()) return
    try {
      const remote = await api.get<Partial<AppSettings>>(`${ADMIN}/settings`)
      this.suppressSave = true
      Object.assign(this.settings, { ...DEFAULT_SETTINGS, ...(remote || {}), network: { ...DEFAULT_SETTINGS.network, ...((remote as any)?.network || {}) } })
      this.serverLoaded = true
      localStorage.setItem(SETTINGS_STORAGE_KEY, JSON.stringify(this.settings))
      this.notifyListeners()
    } catch (error) {
      console.warn('从服务端加载设置失败:', error)
    } finally {
  ...
  async flush(): Promise<void> {
    ...
    if (!getAdminToken()) return
    await api.put(`${ADMIN}/settings`, JSON.parse(JSON.stringify(this.settings)))
    this.serverLoaded = true
  }
  ...
  private saveSettings(): void {
    ...
    this.saveTimer = setTimeout(() => {
      this.saveTimer = null
      api.put(`${ADMIN}/settings`, JSON.parse(JSON.stringify(this.settings)))
        .catch(e => console.warn('写入服务端设置失败:', e))
    }, 400)
  }
  ...
  getSettingsInfo(): Record<keyof AppSettings, string> {
    return {
      ...
      algorithms: '算法配置列表',
      adminToken: '管理员令牌',
      publicQueryRequireToken: '公网题库接口是否需要令牌',
      multiUser: '多用户配置',
...
export function useSettings() {
  return {
    ...
    init: settingsManager.init.bind(settingsManager),
    flush: settingsManager.flush.bind(settingsManager),

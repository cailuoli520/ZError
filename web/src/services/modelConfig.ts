...
import { api, ADMIN, getAdminToken } from './api'
...
const fetchRemoteModelsCatalogFromUrl = async (url: string): Promise<RemoteModelsCatalog> => {
  // 优先走服务端代理（规避 CORS），失败时回退浏览器直连
  try {
    return normalizeRemoteModelsCatalog(await api.get(`${ADMIN}/remote-catalog`))
  } catch (proxyError) {
    try {
      ...
      throw browserError instanceof Error ? browserError : proxyError
    }
  }
}
...
class ModelConfigManager {
  ...
  /**
   * 登录后从服务端加载模型配置（服务端为唯一权威存储，localStorage 仅作缓存）
   */
  async init(): Promise<void> {
    if (!getAdminToken()) return
    try {
      const remote = await api.get<Partial<ModelSettings>>(`${ADMIN}/model-settings`)
      const merged: ModelSettings = {
        ...DEFAULT_MODEL_SETTINGS,
        ...(remote || {}),
        platforms: sanitizeStoredPlatforms(((remote && remote.platforms) || []).filter((p: AIPlatform) => p.name?.trim())),
      }
      this.suppressSave = true
      // 清空后整体替换，保持 reactive 引用不变
      ...
      console.warn('从服务端加载模型配置失败:', error)
    } finally {
      // 等待 watch 回调跑完再恢复保存
  ...
  /**
   * 防抖写回服务端（仅在已从服务端加载过后，避免用本地缓存覆盖服务端）
   */
  private scheduleServerSave(): void {
    if (!this.serverLoaded || !getAdminToken()) return
    if (this.saveTimer) clearTimeout(this.saveTimer)
    this.saveTimer = setTimeout(() => {
      this.saveTimer = null
      api.put(`${ADMIN}/model-settings`, JSON.parse(JSON.stringify(this.settings)))
        .catch(e => console.warn('写入服务端模型配置失败:', e))
    }, 400)
  }
  ...
  async flush(): Promise<void> {
    ...
    if (!getAdminToken()) return
    await api.put(`${ADMIN}/model-settings`, JSON.parse(JSON.stringify(this.settings)))
    this.serverLoaded = true
  }
...
export function useModelConfig() {
  ...
  return {
    ...
    init: modelConfigManager.init.bind(modelConfigManager),
    flush: modelConfigManager.flush.bind(modelConfigManager),

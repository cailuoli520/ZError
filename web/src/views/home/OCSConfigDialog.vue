    ...
    <div class="dialog-panel ocs-config-panel" @click.stop>
      ...
      <div class="dialog-body">
        ...
        <div class="config-info">
          ...
          <span v-if="queryToken">查询地址: <strong>{{ queryUrl }}</strong></span>
          <span v-else class="config-warning">尚未配置查询令牌，请先在「常规设置」中添加查询令牌</span>
          <button class="test-btn" :disabled="!queryToken || testing" @click="handleTest">
            {{ testing ? '测试中...' : '测试连接' }}
          </button>
        </div>
        <div v-if="testMessage" class="test-result" :class="{ ok: testOk, fail: !testOk }">{{ testMessage }}</div>
      </div>
    </div>
  </div>
</template>
...
/** 查询令牌：取多用户配置中的第一个用户令牌 */
const queryToken = computed(() => settings.multiUser?.users?.[0]?.token || '')
const queryUrl = computed(() => `${location.origin}/query?token=${queryToken.value}`)
...
/** 测试连接：以 title=test 请求一次公开查询接口 */
const handleTest = async () => {
  if (!queryToken.value || testing.value) return
  testing.value = true
  testMessage.value = ''
  try {
    const response = await fetch(`${queryUrl.value}&title=test`)
    const data = await response.json().catch(() => null)
    if (response.ok) {
      testOk.value = true
      testMessage.value = `测试成功：HTTP ${response.status}${data?.message ? `，${data.message}` : ''}`
    } else {
      testOk.value = false
      testMessage.value = `测试失败：HTTP ${response.status}${data?.message ? `，${data.message}` : ''}`
    }
  } catch (error) {
    testOk.value = false
    testMessage.value = `测试失败：${error instanceof Error ? error.message : '未知错误'}`
  } finally {
    testing.value = false
  }
}
...
.test-btn:hover:not(:disabled) { background: var(--bg-tertiary); }
...
.test-result.ok { color: #38a169; }
.test-result.fail { color: var(--color-error, #e53e3e); }
</style>

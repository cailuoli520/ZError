...
const importSoftwareExportedFile = () => {
...
const handleImportFileChange = async (event: Event) => {
  ...
  loading.value = true;
  try {
    const items = await parseSoftwareExportedFile(file);
    // 触发全局事件打开导入对话框
    bus.emit('open-import-dialog', { items });
  } catch (e) {
...
const exportFile = async (format: 'csv' | 'xlsx' | 'docx' | 'pdf' | 'txt') => {
  ...
  try {
    ...
    const fileName = `${folderName}_${currentDate}.${format}`;
...
const performSearch = async () => {
  ...
  try {
    ...
    // 获取用于高亮的关键词（进行分词）
    try {
      const segments = await databaseService.segmentText(searchTerm.value.trim());

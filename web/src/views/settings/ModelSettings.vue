...
import { resolveRuntimeModelId } from '../../services/modelProtocol'
import { postSse } from '../../services/api'
...
const getRemoteModelIconBaseUrl = (): string => PROD_REMOTE_MODEL_ICON_BASE_URL
...
const testModel = async (model: AIModel, testFunctionCalling: boolean = false) => {
  ...
  try {
    ...
    if (model.category === 'vision' && !testFunctionCalling) {
      ...
      let imageBase64 = ''
      try {
        const imageUrl = '/assets/images/vlm_test/vlm_test.png'
        // 使用确定的URL获取图片
    ...
    await postSse('/api/admin/models/test', {
      modelId: model.id,
      messages: testInput.messages,
    }, (event, data) => {
      if (event === 'chunk') {
        if (firstTokenAt === null && (data?.content || data?.reasoning_content)) firstTokenAt = performance.now()
        ...
        fullReasoning = data?.reasoning_content || fullReasoning
        if (data?.usage) lastUsagePayload = { usage: data.usage }
      } else if (event === 'error') {
        serverError = data?.message || '模型调用失败'
      }
    }, testAbortController.value?.signal)

    if (serverError) {
      currentTestResult.value = null
      currentTestError.value = `测试失败: ${serverError}`
    } else if (!hasVisibleOutputContent(fullResponse)) {
      currentTestResult.value = null
      ...
      currentTestResult.value = {
        ...
        testType: model.category === 'vision' ? '图像理解测试' : (testFunctionCalling ? '函数调用测试（服务端不注入工具，仅验证连通）' : '文本对话测试')
      }
    }
  } catch (error) {

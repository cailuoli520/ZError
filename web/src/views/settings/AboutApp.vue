    ...
    <div class="network-group">
      ...
      <div class="app-info-item">
        ...
        <div class="app-actions">
          <span class="update-status success">服务端版本</span>
        </div>
      </div>
    </div>
...
const loadVersion = async () => {
  try {
    const res = await api.get<{ version: string }>(`${ADMIN}/version`)
    currentVersion.value = res.version || '未知'
  } catch {
...
const checkEnvironment = () => {
  debugInfo.value = {
    ...
    availableApis: ['fetch', 'EventSource']
  }
}
...
const loadRequestLogs = async () => {
  ...
  try {
    // 服务端返回 [["2026-03-31", 5], ["2026-04-01", 3], ...]
    const rows = await api.get<[string, number][]>(`${ADMIN}/stats/daily`)

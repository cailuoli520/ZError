...
const fetchModels = async () => {
  ...
  try {
    // 由服务端代理探测 /models 与 /v1/models
    let models: FetchedModelItem[] = []
    try {
      const res = await api.post<{ models: string[] }>(`${ADMIN}/platforms/probe-models`, {
        baseUrl,
        apiKey: props.platformApiKey || '',
      })
      models = (res.models || []).map(id => ({ id, ownedBy: '' }))
    } catch (error: any) {
...
const createDialogFormData = (model?: AIModel | null) => {
  // 服务端不再执行自定义 JS，custom 一律按 openai-chat 处理
  const rawProtocol = normalizeApiProtocol(model?.apiProtocol)
  const protocol = rawProtocol === 'custom' ? 'openai-chat' : rawProtocol
  ...
  return {
    ...
    category,
    jsCode: buildPresetProcessModelJsCode({
        protocol,
        modelId,
        enableThinking,
